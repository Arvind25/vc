define(function(require) {
	...
	flipboard.init = function (display_spec, custom, perms) {
		...
		var page = Page();
		page.init ();
		content.init(f_handle);
		events.bind ('contentupload', conversion_successfullhandler, 'uploadHandler');
	...
	flipboard.info = function (from, id, data) {
		console.log('Broadcaset Flipboard response:--> ', from);
		if(id === 'draw'){
			draw(data);
		}else if(id === 'content'){
			content.info(id,data);
		}else{
	...
	conversion_successfullhandler = function (ev, data) {
		if(ev === 'success'){
			console.log('Flipboard conversion success event.');
			content.load_player(data);		
		}
	};
	function draw (data) {
